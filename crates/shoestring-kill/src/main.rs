...
//! then sends [`Request::CloseWindow`] to close the chosen toplevel.
...
fn run() -> Result<u8> {
    ...
    eprintln!("shoestring-kill: click a window to close it (Escape or right-click to cancel)");
    ...
    match send(
        ...
        Response::Ok => {
            eprintln!("shoestring-kill: closed {label}");
            Ok(EXIT_OK)
        }
        Response::Error { message } => {
            anyhow::bail!("close_window {:?}: {message}", window.id);
        }
        other => anyhow::bail!("close_window: unexpected reply {other:?}"),
    }
}
